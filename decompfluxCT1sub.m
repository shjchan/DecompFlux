function [yn,p2,Rz,Cor] = decompfluxCT1sub(ST,fm,lb0,flux,big,coeff,block,SM)
%[yn,p2,Rz,Cor] = decompfluxCT1sub(ST,fm,lb0,flux,big,coeff,block)
%Check the decomposability of flux mode fm by MILP using Cplex solver
%Using the constraint "p_j<=M(v_j)(a_j)" where M is a big number and v_j is
...
Aineq=sparse([[zeros(2*Nm,Nr) [ST;-ST]];...
       [eye(Nr)        -eye(Nr)];...
       [-diag((big/mpos)*flux)  eye(Nr)];...
       [ones(1,Nr)  zeros(1,Nr)];... 
       [-ones(1,Nr) zeros(1,Nr)];... 
       BK;...
       ]);
if isempty(BK)
    bineq=[epsIMB*ones(2*Nm,1);zeros(2*Nr,1);sum(fm>eps8)-1;-1];
else
    bineq=[epsIMB*ones(2*Nm,1);zeros(2*Nr,1);sum(fm>eps8)-1;-1;sum(logical(block'),2)-1];
end
ubp=(big/mpos)*flux;
ubp(fm<=eps8)=0;
uba=ones(Nr,1);
uba(fm<=eps8)=0;
lba=zeros(Nr,1);
ctype=char([ones(1,Nr)*'B' ones(1,Nr)*'C']);
options=cplexoptimset;
options.TolFun=10^(-8);
options.TolRLPFun=10^(-8);
options.TolXInteger=10^(-8);
[sol,fval,flag,out]=cplexmilp(f,Aineq,bineq,[],[],[],[],[],...
        [lba;lb0],[uba;ubp],ctype,[],options);
if SM >= 3
     disp(out);
end
Cor=[];

function [EFM,FM,Rshut,Reduce,Corr,depth] = decompfluxCT2(ST,flux0,big,lb0,ub0,coeff,block,SM,solver,param)
%[EFM,FM,Rshut,Reduce,Corr,depth] = decompfluxCT2(ST,flux0,big,lb0,ub0,coeff,block)
%Decompose a flux distribution into a set of EFM
%In the subproblem DC, "p_j<=M(delta_j)(a_j)" is used where M is a big number
%
%Input:
...
%big:   the large number M used in the subproblem 
...
while cur>0
    ...
    if sum(art>eps0)>0
        if strcmp(solver,'cplex')
            [yn,p2,Rz,Cor] = decompfluxCT2sub(ST,art,lb0,PWtree(:,1),big,coeff,block,SM);
        elseif strcmp(solver,'cobra')
            if exist('param','var')
                [yn,p2,Rz,Cor] = decompfluxCT2subCOBRA(ST,art,lb0,PWtree(:,1),big,coeff,block,SM,param);
            else
                [yn,p2,Rz,Cor] = decompfluxCT2subCOBRA(ST,art,lb0,PWtree(:,1),big,coeff,block,SM);
            end
        else
...
FM=FM(:,1:Nfm);
for j = 1:size(FM,2);
    r = min(flux0(FM(:,j)~=0)./FM(FM(:,j)~=0,j));
    FM(:,j) = FM(:,j) * r;
end
